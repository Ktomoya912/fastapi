...
!!! 情報
    ...
    現在のドキュメントは、Pydantic v1、SQLAlchemy version 2.0 未満を想定しています。

    新しいドキュメントは Pydantic v2 が含まれており、<a href="https://sqlmodel.tiangolo.com/" class="external-link" target="_blank">SQLModel</a> (SQLAlchemyをベースにしています。) が Pydantic v2 に対応したときにアップデート予定です。

**FastAPI** は SQL (リレーショナル) データベースを要求しません。

しかし、あなたが望むリレーショナルデータベースを使用することができます。

ここに <a href="https://www.sqlalchemy.org/" class="external-link" target="_blank">SQLAlchemy</a> を使用した例があります。

SQLAlchemy でサポートされているデータベースを簡単に使用することができます。例えば:

* PostgreSQL
* MySQL
* SQLite
* Oracle
* Microsoft SQL Server, などなど...

今回の例では、 **SQLite** を使用します。なぜなら、それは単一のファイルを使用し、Python に統合されたサポートがあるからです。そのため、この例をコピーしてそのまま実行することができます。

のちの本番環境では、**PostgreSQL** のようなデータベースサーバーを使用することをお勧めします。
...
!!! 備考
    コードの大部分はフレームワークで使用する標準の `SQLAlchemy` コードであることに注意してください。

    **FastAPI** のコードはいつもと同じように小さいです。

## ORMs

**FastAPI** はどんなデータベースでも、どんなスタイルのライブラリでも動作します。

一般的なパターンは、"ORM" (object-relational mapping) ライブラリを使用することです。

ORM には、コード内の *objects* とデータベーステーブル ("*relation*") の間を変換 ("*map*") するためのツールがあります。

ORM を使用すると、通常、SQL データベースのテーブルを表すクラスを作成します。クラスの各属性は、名前と型を持つ列を表します

例えば、 `Pet` クラスは SQL テーブル `pets` を表すことができます。

そして、そのクラスの各 *インスタンス* オブジェクトは、データベースの行を表します。

例えば、 `orion_cat` オブジェクト ( `Pet` のインスタンス) は、 `type` 列のための属性 `orion_cat.type` を持つことができます。そして、その属性の値は、例えば `"cat"` になります。

ORM は、通常、テーブルまたはエンティティ間の接続または関係を作成するためのツールも持っています。

このようにすると、`oriom_cat.owner` という属性を持つことができ、owner はテーブル *owners* から取得したこのペットの飼い主のデータを含むことができます。

そのため、 `orion_cat.owner.name` は、このペットの飼い主の `owners` テーブルの `name` 列から取得した名前 ( `name` 列) となります。
